...
Forge can run from Docker, from the prepared npm package metadata, or from a source checkout. For release readiness, treat Docker as the universal install path, npm as the package distribution path once published, and git clone as the source install path.

## Platform Support

| Platform | Support stance |
|----------|----------------|
| Docker | Recommended universal path across macOS, Linux, Windows, NAS appliances, and servers. |
| macOS native | Supported for source installs and development with Node.js 22.6+. |
| Linux native | Supported for source installs and server deployment with Node.js 22.6+. Ubuntu/Debian plus systemd is the primary bare-metal server shape. |
| Windows via Docker or WSL | Recommended Windows path. Docker Desktop and WSL keep the runtime close to the Linux/macOS paths. |
| Windows native | CI-tracked and experimental until native install, build, provider subprocesses, and runtime behavior are validated end to end. |
...
Docker is the recommended path when you want repeatable deployment across machines.
...
The package metadata is prepared for the scoped package `@snootypants/forge`, with the installed CLI command mapped to `forge`. Registry availability depends on publishing the package. The unscoped `forge` npm package name is currently occupied, so use the scoped package name for installs.

After the package is published:

```bash
npm install -g @snootypants/forge
forge init
